...
<div class="card bg-dark mb-3">
  <div class="card-header text-white">
    <i class="fa fa-edit text-warning"></i> {$vod_title}{$play_title}
  ...
  <div class="card-body text-break pt-2 pb-0">
    ...
    <a class="mr-1 text-light" href="{:DcUrl('maccms/filter/area',['wd'=>$area])}">{$area}</a>/
    {/volist}
    {volist name="vod_year" id="year"}
    <a class="mr-1 text-light" href="{:DcUrl('maccms/filter/year',['wd'=>$year])}">{$year}</a>/
    {/volist}
    {volist name="vod_language" id="language"}
    <a class="mr-1 text-light" href="{:DcUrl('maccms/filter/language',['wd'=>$language])}">{$language}</a>/
    {/volist} 
    <span class="text-light">{$vod_updatetime|maccmsDate='Y-m-d',###}</span>
  </div>
  <div class="card-body pt-2 pb-0">
    {volist name="vod_actor" id="actor"}<a class="mr-2 text-light" href="{:DcUrl('maccms/filter/actor',['wd'=>$actor])}">{$actor}</a>{/volist}
    {volist name="vod_director" id="director"}<a class="mr-2 text-light" href="{:DcUrl('maccms/filter/director',['wd'=>$director])}">{$director}</a>{/volist}
  </div>
