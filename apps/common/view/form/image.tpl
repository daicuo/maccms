<div class="{$form.class|default='row form-group'}">
  ...
  <div class="{$form.class_right|default='col-md-6'}">
    <div class="input-group">
      <input {if $form['readonly']}readonly{/if} {if $form['disabled']} disabled{/if} {if $form['required']} required{/if} {if $form['autofocus']} autofocus{/if} type="text" class="{$form.class_right_control|default='form-control form-control-sm'}" id="{:DcEmpty($form['id'],$form['name'])}" name="{$form.name}" value="{$form.value|DcHtml}" placeholder="{$form.placeholder}" maxlength="{$form.maxlength}" autocomplete="{$form.autocomplete|DcSwitch}">
      <div class="input-group-append">
        <button class="btn btn-sm btn-outline-secondary dc-image-preview {if $form['value']}d-inline-block{else/}d-none{/if}" type="button" data-toggle="imagePreview" data-target="#{:DcEmpty($form['id'],$form['name'])}" data-cdn="{:config('common.upload_cdn')}" data-root="{:DcRoot()}" data-path="{:config('common.upload_path')}">{:lang('preview')}</button>
