<div class="{$form.class|default='row form-group'}">
  ...
  <div class="{$form.class_right|default='col-md-6'}">
    <div class="input-group dc-datetime" id="dc-datetime-{$form.id}" data-target-input="nearest" data-toggle="datetime">
      <input type="text" class="form-control datetimepicker-input" id="{$form.id}" name="{$form.name}" value="{$form.value|DcHtml}" placeholder="{$form.placeholder}" maxlength="{$form.maxlength}" autocomplete="{$form.autocomplete|DcSwitch}" data-target="#dc-datetime-{$form.id}" {if $form['readonly']}readonly{/if} {if $form['disabled']} disabled{/if} {if $form['required']} required{/if} {if $form['autofocus']} autofocus{/if}/>
